...
static u64 Solve(const char* fileName)
{
	...
	while (!coordsQueue.empty())
	{
		...
		if (coordX > 0)
		{
			QueueElement& newLeftElem = coordsQueue.emplace();
			newLeftElem.matrixCoord = coordY * colCount + coordX - 1;
			newLeftElem.partialSum = elemPartialSum;
		}
	...
	// bottom-left cell
	return partialSumsMatrix.back();
}

int main()
{
	constexpr const char* fileName = "resources/0083_matrix.txt";
	const int sol = Solve(fileName);
	std::cout << "Solution 83: " << sol << std::endl;
